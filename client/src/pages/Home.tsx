...
type Drawer = "chat" | "memory" | "journey" | "project" | "focus" | "career" | "demo" | "lab" | "portfolio" | "report" | "learning" | null;
...
const lessonStages = [
  { id: "learn", label: "Learn", title: "Name the idea", body: "Hana gives you the smallest useful explanation first.", prompt: "Want the short version or a tiny analogy?" },
  { id: "try", label: "Try", title: "Touch the idea", body: "Make one choice, sketch one request, or answer one friendly question.", prompt: "Want a hint, or would you like to try first?" },
  { id: "build", label: "Build", title: "Make it useful", body: "Turn the concept into a small thing someone could actually use.", prompt: "Should we make the interface, the data flow, or the first test?" },
  { id: "check", label: "Check", title: "Show what clicked", body: "Explain it back to Hana or choose the next kind correction.", prompt: "What feels clearer now?" },
] as const;
...
export default function Home() {
  ...
  const [learningMode, setLearningMode] = useState<"learn" | "build">("learn");
  ...
  const careerRecommendation = useMemo(() => {
    const text = [...careerAnswers.map((a) => a.answer), ...completed.map((id) => quests.find((quest) => quest.id === id)?.topic ?? "")].join(" ").toLowerCase();
  ...
  return (
    <AppShell active={active} onNav={nav}>
      <div className="container py-8 pb-28 lg:py-12 lg:pb-16">
        ...
        </section><section className="mt-8 grid gap-6 lg:grid-cols-[1.1fr_.9fr]"><article className="paper-card rounded-[26px] bg-[#e3ecd9] p-6"><div className="flex items-center justify-between"><div><p className="text-xs font-bold uppercase tracking-[.18em] text-[#56704f]">Skill garden</p><h2 className="mt-2 font-display text-2xl">Grow what you can demonstrate.</h2></div><Sprout size={21} className="text-[#6f8c68]"/></div><div className="mt-4 grid grid-cols-2 gap-2">{skillGarden.map((skill) => <button key={skill.name} onClick={() => setExpandedInfo(expandedInfo === skill.name ? null : skill.name)} className="rounded-2xl p-3 text-left" style={{ background: skill.color }}><span className="text-xl">{skill.plant}</span><span className="mt-1 block text-xs font-bold text-[#52634d]">{skill.name}</span><span className="mt-2 block h-1.5 rounded-full bg-[#fffaf1]/70"><span className="block h-1.5 rounded-full bg-[#56704f]" style={{ width: `${skill.value}%` }}/></span>{expandedInfo === skill.name && <span className="mt-2 block text-[10px] leading-relaxed text-[#60705b]">Hana grows this when you practice or build with {skill.name.toLowerCase()}. Click again to tuck away.</span>}</button>)}</div></article><article className="paper-card rounded-[26px] bg-[#e4edf3] p-6"><p className="text-xs font-bold uppercase tracking-[.18em] text-[#56709a]">Explore</p><h2 className="mt-2 font-display text-2xl">Learn by making.</h2><p className="mt-2 text-sm leading-relaxed text-[#50627b]">Short answer first. Click when you want the deeper door.</p><div className="mt-4 flex flex-wrap gap-2"><button onClick={() => setDrawer("lab")} className="rounded-full bg-[#fffaf1] px-4 py-2 text-xs font-bold text-[#56709a]">Hana’s Lab</button><button onClick={() => setDrawer("portfolio")} className="rounded-full bg-[#fffaf1] px-4 py-2 text-xs font-bold text-[#56709a]">Portfolio</button></div></article></section><section className="mt-8 grid gap-6 lg:grid-cols-[1.1fr_.9fr]"><article className="paper-card rounded-[26px] bg-[#fffaf1] p-6"><p className="text-xs font-bold uppercase tracking-[.18em] text-[#8b7764]">Where am I going?</p><h2 className="mt-2 font-display text-2xl">Today’s skill has somewhere to go.</h2><div className="mt-5 flex flex-wrap items-center gap-2 text-xs font-bold text-[#6d6258]"><span className="rounded-full bg-[#e3ecd9] px-3 py-2">{selectedQuest.topic}</span><ArrowRight size={14}/><span className="rounded-full bg-[#dce7f4] px-3 py-2">{recommendedPath}</span><ArrowRight size={14}/><span className="rounded-full bg-[#f2e6b8] px-3 py-2">A real project</span></div><button onClick={() => continueWithHana(`This is the thread I’m holding: ${selectedQuest.topic} can become a ${recommendedPath} project. Want to see the next bridge?`, "curious")} className="mt-5 text-sm font-bold text-[#3157c8]">Ask Hana to connect it <ArrowRight size={15} className="ml-1 inline"/></button></article><article className="paper-card rounded-[26px] bg-[#f0e1df] p-6"><p className="text-xs font-bold uppercase tracking-[.18em] text-[#8d5961]">Weekly Hana review</p><h2 className="mt-2 font-display text-2xl">You are growing a pattern.</h2><p className="mt-3 text-sm leading-relaxed text-[#765f60]">You touched {selectedQuest.topic.toLowerCase()}, made one choice, and kept your place. That is enough evidence for a thoughtful next step.</p><button onClick={() => setDrawer("report")} className="mt-5 rounded-full bg-[#2f2a25] px-4 py-2 text-xs font-bold text-white">Open my weekly report <ArrowRight size={14} className="ml-1 inline"/></button><button onClick={() => { setEnergyMode("light"); continueWithHana("You did enough for today. I’ll keep the next step safe until you’re ready to return.", "sleepy"); }} className="ml-2 mt-5 rounded-full bg-[#fffaf1]/70 px-4 py-2 text-xs font-bold text-[#8d5961]">Enough for today</button></article></section></div>
      ...
      {drawer && <div className="fixed inset-0 z-50 flex items-end justify-end bg-[#2f2a25]/25 backdrop-blur-[2px]" onClick={() => setDrawer(null)}><aside className="max-h-[92vh] w-full overflow-y-auto rounded-t-[30px] bg-[#fffaf1] p-5 shadow-2xl sm:max-w-[560px] sm:rounded-l-[30px] sm:rounded-r-none sm:p-7" onClick={(e) => e.stopPropagation()}>
        <div className="mb-6 flex items-center justify-between"><div className="flex items-center gap-3"><span className="grid size-11 place-items-center rounded-2xl bg-[#e3ecd9]"><img src={ASSETS.mark} alt="" className="size-8" /></span><div><p className="text-xs font-bold uppercase tracking-[.16em] text-[#8b7764]">{drawer === "chat" ? "Ask Hana" : drawer === "memory" ? "Hana remembers" : drawer === "journey" ? "Hana Journey" : drawer === "project" ? "Hana Projects" : drawer === "career" ? "Find My Career Path" : drawer === "demo" ? "Try Hana" : drawer === "lab" ? "Hana’s Lab" : drawer === "portfolio" ? "Hana Portfolio" : drawer === "report" ? "Weekly Hana Report" : "Hana Focus"}</p><h2 className="font-display text-2xl">{drawer === "chat" ? "Let’s make it clearer." : drawer === "memory" ? "Useful context, in your hands." : drawer === "journey" ? "Choose your next quest." : drawer === "project" ? "Turn learning into a thing." : drawer === "career" ? "Let’s find your direction." : drawer === "demo" ? "A tiny tour of Hana." : drawer === "lab" ? "Curiosity is allowed here." : drawer === "portfolio" ? "The things you made matter." : drawer === "report" ? "A gentle look back." : "A calm pocket of time."}</h2></div></div><button onClick={() => setDrawer(null)} className="grid size-10 place-items-center rounded-full bg-[#f3eadc] text-[#6f6359]" aria-label="Close panel"><X size={18}/></button></div><div className="mb-5 rounded-2xl bg-[#f7f1e4] p-4"><div className="flex items-start gap-3"><HanaImage mood={mood} className="size-12 shrink-0"/><p className="text-sm leading-relaxed text-[#5f554c]">{hanaPrompt}</p></div><div className="mt-3 flex flex-wrap gap-2"><button onClick={() => continueWithHana("I’ll give you one small hint, not the whole answer. What part feels foggiest?", "worried")} className="rounded-full bg-[#fffaf1] px-3 py-2 text-xs font-bold text-[#6d6258]">Give me a hint</button><button onClick={() => continueWithHana("Let’s try it together. You can make a first attempt and I’ll stay close.", "curious")} className="rounded-full bg-[#fffaf1] px-3 py-2 text-xs font-bold text-[#6d6258]">Let me try</button><button onClick={() => continueWithHana("I can open one deeper layer. Which part should we look at?", "thinking")} className="rounded-full bg-[#fffaf1] px-3 py-2 text-xs font-bold text-[#6d6258]">Tell me more</button><button onClick={() => continueWithHana("Of course. I’ll explain it another way, with a picture in words and one tiny example. Which part felt unclear?", "confused")} className="rounded-full bg-[#fffaf1] px-3 py-2 text-xs font-bold text-[#6d6258]">Explain another way</button><button onClick={addChatMemory} className="rounded-full bg-[#fffaf1] px-3 py-2 text-xs font-bold text-[#6d6258]">Save this</button></div></div>

        {drawer === "learning" && <div><div className="mb-5 rounded-[24px] bg-[#e3ecd9] p-5"><div className="flex items-start gap-3"><HanaImage mood={lessonStage === 3 ? "proud" : lessonStage === 1 ? "curious" : "thinking"} className="size-20 shrink-0"/><div><p className="text-xs font-bold uppercase tracking-[.16em] text-[#56704f]">Hana’s active lesson</p><h3 className="mt-2 font-display text-3xl">{lessonStages[lessonStage].title}</h3><p className="mt-2 text-sm leading-relaxed text-[#52634d]">{lessonStages[lessonStage].body}</p><p className="mt-3 text-sm font-semibold text-[#56704f]">{lessonStages[lessonStage].prompt}</p></div></div></div><div className="mb-4 rounded-2xl border border-[#dfe4d8] bg-[#fffaf1] p-4"><p className="text-xs font-bold uppercase tracking-[.16em] text-[#8b7764]">Prerequisite thread</p><p className="mt-2 text-sm text-[#6d6258]">Python basics <ArrowRight size={13} className="mx-1 inline"/> functions <ArrowRight size={13} className="mx-1 inline"/> APIs <ArrowRight size={13} className="mx-1 inline"/> automation <ArrowRight size={13} className="mx-1 inline"/> AI agents</p><button onClick={() => continueWithHana("We don’t need to unlock everything today. If one earlier piece feels wobbly, we can review it without losing your place. Which link should we revisit?", "worried")} className="mt-3 text-xs font-bold text-[#8d5961]">Check a foundation with Hana</button></div><div className="grid grid-cols-2 gap-2 sm:grid-cols-4">{lessonStages.map((stage, index) => <button key={stage.id} onClick={() => { setLessonStage(index); continueWithHana(`${stage.label} it is. ${stage.prompt}`, index === 3 ? "proud" : "thinking"); }} className={`rounded-2xl p-3 text-left ${index === lessonStage ? "bg-[#3157c8] text-white" : "bg-[#f7f1e4] text-[#6d6258]"}`}><span className="block text-xs font-bold">0{index + 1}</span><span className="mt-1 block text-sm font-bold">{stage.label}</span></button>)}</div><div className="mt-5 rounded-2xl bg-[#f7f1e4] p-4"><p className="text-xs font-bold uppercase tracking-[.15em] text-[#8b7764]">Your turn</p><p className="mt-2 text-sm leading-relaxed text-[#5f554c]">Tell Hana what feels clearer in your own words. It does not need to sound polished.</p><textarea value={teachText} onChange={(e) => setTeachText(e.target.value)} placeholder="I think an API is…" className="mt-3 min-h-[90px] w-full resize-none rounded-2xl border border-[#dfd3bf] bg-[#fffaf1] p-3 text-sm outline-none focus:border-[#83a77a]"/><div className="mt-3 flex flex-wrap gap-2"><button onClick={() => continueWithHana(teachText.trim() ? `I heard you say: “${teachText.trim()}” — that’s a real explanation, not a copy. Want to build one small version?` : "Give me two sentences in your own words first. I’ll listen for the connection, not perfect phrasing.", teachText.trim() ? "proud" : "curious")} className="rounded-full bg-[#2f2a25] px-4 py-2 text-xs font-bold text-white">Share with Hana</button><button onClick={() => continueWithHana("No problem. I’ll say it another way with a small example. Which word should I slow down on?", "confused")} className="rounded-full bg-[#fffaf1] px-4 py-2 text-xs font-bold text-[#6d6258]">Explain another way</button></div></div></div>}

        {drawer === "report" && <div><div className="mb-5 rounded-[24px] bg-[#f2e6b8] p-5"><p className="text-xs font-bold uppercase tracking-[.16em] text-[#846d34]">This week with Hana</p><h3 className="mt-2 font-display text-3xl">A little evidence of becoming.</h3><p className="mt-2 text-sm leading-relaxed text-[#75632e]">No leaderboards. Just the things you understood, tried, and made.</p></div><div className="space-y-3">{[{label:"Learned",value:`${selectedQuest.topic} · one clear concept`},{label:"Practiced",value:"A small request-and-response story"},{label:"Built",value:"Study Sprint Coach project seed"},{label:"Strongest signal",value:recommendedPath}].map((item) => <button key={item.label} onClick={() => continueWithHana(`${item.label}: ${item.value}. I’m keeping this as a useful thread, not a score. What would you like to carry forward?`, "proud")} className="flex w-full items-center justify-between rounded-2xl bg-[#fffaf1] p-4 text-left"><span><span className="block text-xs font-bold uppercase tracking-[.13em] text-[#8b7764]">{item.label}</span><span className="mt-1 block font-bold text-[#5c5147]">{item.value}</span></span><ChevronRight size={16} className="text-[#846d34]"/></button>)}</div><button onClick={() => setDrawer("portfolio")} className="mt-5 rounded-full bg-[#2f2a25] px-4 py-2 text-xs font-bold text-white">Turn this into portfolio notes <ArrowRight size={14} className="ml-1 inline"/></button></div>}
