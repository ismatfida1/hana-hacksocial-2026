...
function AppShell({ destination, setDestination, plan, mission, missionWhy, started, setStarted, chatMessages, sendChat, chatLoading }: { destination: Destination; setDestination: (value: Destination) => void; plan: Plan | null; mission: string; missionWhy: string; started: boolean; setStarted: (value: boolean) => void; chatMessages: Message[]; sendChat: (value: string) => void; chatLoading: boolean }) {
  ...
  const renderContent = () => {
    if (destination === "Journey") return <section className="space-y-5 pb-8"><div><p className="text-sm font-semibold text-[#C98C93]">Your Journey</p><h1 className="mt-2 font-display text-4xl text-[#3A3540]">Start today. Build steadily.</h1><p className="mt-2 text-sm leading-5 text-[#625D65]">{todayText()} · Full-day plan · {plan?.area || "Your chosen direction"}</p></div><div className="rounded-3xl border border-[#D8D4E4] bg-[#EDECF6] p-4"><div className="flex items-center gap-2 text-sm font-bold text-[#51486A]"><BookOpen size={17} /> Today’s finish line</div><p className="mt-2 font-display text-2xl text-[#3A3540]">{steps[activeStepIndex]?.title}</p><p className="mt-1 text-sm text-[#625D65]">{steps[activeStepIndex]?.purpose}</p>{steps[activeStepIndex]?.resource && <a href={steps[activeStepIndex].resource.url} target="_blank" rel="noreferrer" className="mt-3 inline-flex items-center gap-2 rounded-xl bg-white px-3 py-2 text-sm font-bold text-[#3157C8] underline underline-offset-4">Study this here: {steps[activeStepIndex].resource.label}<ExternalLink size={15} /></a>}</div><div className="space-y-3">{steps.map((step, index) => <div key={`${step.title}-${index}`} onClick={() => setSelectedStep(index)}><DetailStep step={step} index={index} active={index === selectedStep || index === activeStepIndex} /></div>)}</div></section>;
