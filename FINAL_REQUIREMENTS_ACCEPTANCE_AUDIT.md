...
| Project evidence feeds portfolio/readiness/opportunities | **PARTIAL** | Inspected project, portfolio, career-readiness, opportunity helpers, derived project-progress summary, and focused Projects view | The unified context and Projects view now expose the active project gate, evidence-based difficulty, completed projects, portfolio-ready evidence, and a safe next opportunity action from persisted milestones. There is still no complete automatic submission/propagation pipeline into every downstream system. |
...
| AI data minimization | **VERIFIED at formatter/test level** | Ran `context-minimization.test.ts`, student-context preference coverage, and inspected Ask Hana router call | Learning, university, or work context is selected by question instead of sending the complete stored context every time; the saved explanation style is bounded and included in minimized context. Provider retention and plan terms still require external review. |
...
| Account deletion | **PARTIAL** | Inspected protected delete procedure, public deletion request route, schema, tests, and the managed-storage contract | Technical deletion controls exist: account-owned memory, upload metadata, and user rows are removed; the storage contract states that removing the key from the database and UI references is the supported deletion boundary because no underlying-object delete endpoint is exposed. End-to-end confirmation of provider retention, sessions, backups, conversations, and final release behavior remains open. |
| Teach Hana uploads | **VERIFIED at code/test level** | Inspected protected upload procedures, managed storage path, metadata table, Profile deletion control, security contract test, and storage skill contract | Code/text files are confirmed before upload, limited to 1 MB and known extensions, stored through server-side managed storage, represented in the database as metadata only, and removable by the owning account. The documented storage contract treats removing the key from the database and UI references as the supported deletion boundary; provider retention still requires operational verification. |
